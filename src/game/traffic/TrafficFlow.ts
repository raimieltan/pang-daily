...
export type TrafficActor={follower:WaypointRival;kind:typeof TRAFFIC_TYPES[number];direction:1|-1;active:boolean};
export const SPAWN_CLEARANCE=180;
...
export class TrafficFlow {
 ...
 constructor(lanes:readonly (readonly Waypoint[])[],player:Point,count=18){
  for(let i=0;i<count;i++){
   const direction=i%2===0?1:-1,kind=TRAFFIC_TYPES[i%TRAFFIC_TYPES.length],lane=lanes[direction===1?0:1];
   ...
   this.actors.push({follower,kind,direction,active:distance(follower.position,player)>SPAWN_CLEARANCE});
  }
 }
 ...
 private step(dt:number,player:RoadUser){
  for(const actor of this.actors){
   ...
   if(f.departed){
    if(distance(f.position,player)>SPAWN_CLEARANCE&&clear(f.points[0])){f.reset();actor.active=true;}
    continue;
   }
   ...
   if(limit<.1)continue;
   f.update(dt);f.speed=Math.min(f.speed,limit);
  }
 }
}
