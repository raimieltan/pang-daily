...
import type { ExteriorView, ExteriorInventoryView } from '@/game/vehicles/ExteriorSystem';
...
type State = Board & { wheels: WheelsView | null; exterior: ExteriorView | null; exteriorInventory: ExteriorInventoryView | null };
const board: Board = { view: null, purchase: null, inspection: null, error: null };
const initial: State = { ...board, wheels: null, exterior: null, exteriorInventory: null };
export const useMarketStore = create<State>(() => initial);
const MARKET_COMMANDS = new Set(['openMarketplace', 'buyListing', 'inspectPart', 'equipWheels', 'equipBodyPart', 'removeBodyPart', 'refinishBodyPart']);
...
export function bindMarketStore(events: GameEventSource) {
  ...
  const release = [
    // Closing the board keeps the car's wheel and body part state: it comes from the scene, not the board.
    ...
    events.on('exteriorState', exterior => set({ exterior, error: null })),
    events.on('exteriorInventory', exteriorInventory => set({ exteriorInventory })),
    events.on('sceneLoading', () => set({ wheels: null, exterior: null, exteriorInventory: null, error: null })),
