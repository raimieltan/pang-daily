...
| Exterior sockets | `<slot>_socket`, e.g. `spoiler_socket`, `fender_fl_socket` | no (created by the importer) | Named by `attachments[].socket`. Body parts mount here; see §4 and §8. |
...
**Exterior sockets.** Each slot in `visual.model.attachments` gets a named socket under the
chassis (`attachments[].socket`, by convention `<slot>_socket`, see `exteriorSocketName`), so
parts follow ride height. The slots are `hood`, `bumper_front`, `bumper_rear`, `front_lip`,
`chin`, `side_skirts`, `spoiler`, `fender_fl`, `fender_fr`, `side_mirrors`, `roof`,
`accessory_front`, `accessory_rear`, `exhaust` and the lights. Pairs that are sold as one part
(skirts, mirrors) use one centred socket; fenders have one per side. The vehicle's `tags`
(e.g. `banwa_dalagan`, `sedan`) decide which body parts fit it. The anchor position comes from the first of these that
exists:
...
One file per body part (`assetPath` in `src/game-core/exterior/catalog.ts`), in
`public/model/body/`. Schema and socket list: `src/game-core/exterior/README.md`.

| Convention | Rule |
|---|---|
| Content | Just the part: a lip, a wing with its uprights, one fender. No car. |
| Axes | Car space: +x right, +y up, +z forward, as the car faces. Model it where it sits on the car. |
| Origin | The socket the part mounts on (e.g. `spoiler_socket` at the trunk edge). Look up the socket's position in `catalog.ts` and model relative to it. The part's `transform` can nudge it. |
| Materials | `panel`: the painted/finished surface. The runtime clones it per copy and sets colour, roughness, metalness and clear coat from the finish (body colour, primer, donor paint, bare plastic, fake carbon, damaged) and wear. Anything else (`hardware`: brackets, zip ties, grilles) is shared and left as authored. |
| Budget | ~500 triangles, 2 materials. A car may carry eight or more parts. |
| Pairs | Parts sold as a pair (skirts, mirrors) are one file with both sides. |

Misfit is applied at runtime: a gappy or zip-tied part hangs up to 2.5 cm low and pitches up to
4°, so model the part fitting perfectly. Cracks and scratches are material tints for now, not
geometry.

The placeholder parts are generated by `node scripts/build-body-part-models.mjs` (no
dependencies). `src/game/vehicles/BodyPartSwapper.test.ts` loads them onto the starter sedan.


## Active modular Dalagan

The starter vehicle uses `/model/banwa_dalagan_1996_modular.glb`.
See `public/model/README.md` and `public/model/mounting_interfaces.json`
for the authored mount frames and canonical seams. Source glTF uses +X-left;
Babylon imports it as +X-right, matching the runtime car axes.

Attachment `stockNode` accepts a node name or a group of node names for paired
parts. `mountNode` maps slots such as `spoiler` to `attach_spoiler_rear`.
The stock geometry and replacement both belong to the runtime socket, allowing
the hood to rotate about its cowl hinge during workshop inspection.

`node scripts/build-body-part-models.mjs` rebuilds compatible aftermarket
assets from the stock panel geometry, preserving panel seams and local mount
origins. The modular model has 19,732 triangles, 72 draw calls and 14 materials;
its draw-call budget is 80 to accommodate the independently removable parts.
